...
namespace SharpConfig
{
    ...
    public sealed class Section : ConfigurationElement, IEnumerable<Setting>
    {
        ...
        public static Section FromObject(object obj)
        {
            if (obj == null)
            {
            ...
            var attr = obj.GetType().GetCustomAttribute<SectionAttribute>();
            if (attr != null && !string.IsNullOrEmpty(attr.Name))
            {
                name = attr.Name;
            }

            return FromObject(name, obj);
        }
        ...
        /// <summary>
        /// Performs the specified action on each element of the Section.
        /// </summary>
        /// <param name="action">The System.Action`1 delegate to perform on each element of the Section.</param>
        /// <exception cref="ArgumentNullException"></exception>
        public void ForEach(Action<Setting> action)
        {
            if (action == null)
            {
                throw new ArgumentNullException("action");
            }
            _settings.ForEach(action);
        }
