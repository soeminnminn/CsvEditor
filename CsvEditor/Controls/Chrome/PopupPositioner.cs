...
namespace CsvEditor.Controls
{
    internal class PopupPositioner : DependencyObject, IDisposable
    {
        ...
        private void UpdatePosition()
        {
            ...
            int bestX = Helper.DoubleToInt(bestTranslation.X);
            int bestY = Helper.DoubleToInt(bestTranslation.Y);
        ...
        private class PopupSecurityHelper
        {
            ...
            internal void SetPopupPos(bool position, int x, int y, bool size, int width, int height)
            {
                ...
                NativeMethods.SetWindowPos(new HandleRef(null, Handle), new HandleRef(null, IntPtr.Zero),
            ...
            internal Rect GetWindowRect()
            {
                ...
                if (IsWindowAlive())
                {
                    NativeMethods.GetWindowRect(_window.CreateHandleRef(), ref rect);
                }

                return Helper.ToRect(rect);
            }
        ...
        private static class Delegates
        {
            ...
            static Delegates()
            {
                try
                {
                    GetPlacementInternal = CreatePropertyGetter<Popup, PlacementMode>(
                        nameof(PlacementInternal),
                        BindingFlags.Instance | BindingFlags.NonPublic,
                        true);

                    GetDropOpposite = CreatePropertyGetter<Popup, bool>(
                        nameof(DropOpposite),
                        BindingFlags.Instance | BindingFlags.NonPublic,
                        true);

                    GetPlacementTargetInterestPoints = CreateDelegate<Func<Popup, PlacementMode, Point[]>>(
                        typeof(Popup),
                        nameof(GetPlacementTargetInterestPoints),
                        BindingFlags.Instance | BindingFlags.NonPublic);

                    GetChildInterestPoints = CreateDelegate<Func<Popup, PlacementMode, Point[]>>(
                        typeof(Popup),
                        nameof(GetChildInterestPoints),
                        BindingFlags.Instance | BindingFlags.NonPublic);

                    GetScreenBounds = CreateDelegate<Func<Popup, Rect, Point, Rect>>(
                        typeof(Popup),
